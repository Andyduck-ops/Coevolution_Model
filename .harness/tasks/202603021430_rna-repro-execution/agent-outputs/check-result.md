...
- [x] ablation gate 通过（当前 R1-R4 全通过）
- [ ] integrated paper gate 通过（当前为 false）

## 失败项与修复建议
- 已修复：E1 单调方向、E4 crossing 计数、热力学阈值定义与核心 ODE 结构对齐后，claim gate 已转绿。
- 已修复：R1-R4 消融判据与 post-seg 动力学一致性，ablation gate 已转绿。
- 当前主失败：仅 repro fidelity gate（四图 composite<0.75）。
- 后续建议：继续参数搜索 + 曲线级拟合（point-wise / DTW / key-feature 对齐）。
